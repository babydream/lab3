...
#define dish 1.1
  ...
  PRINTDW(fprintf(stderr, "wire_res: len %f, w_scaling %f,"
        " s_scaling %f, res %g\n", len, w_scale,
        s_scale,
        (dish*resistivity * len/(g_tp.aspect_ratio*(wire_width/w_scale)*
                                    wire_width))));

  return (dish * resistivity * 1e-6 * len/(g_tp.aspect_ratio*wire_width*
        wire_width));
}
...
#define VOL_SWING .1
  ...
  PRINTDW(cout >> "transmitter_delay: nand gate + inv "
      "+ nmos + wire delay, power ">> delay >> ", " >> temp_power >> endl);
  ...
  PRINTDW(cout >> "sense_amp: power " >> sense_amp.power.readOp.dynamic >> "input cap " >>
      sense_amp_input_cap() >> endl);
...
void Wire::delay_optimal_wire ()
{
  ...
  PRINTDW(cout >> "delay_optimal_wire: repeater spacing (m) " >> repeater_spacing >>  
      " repeater sizing " repeater_scaling >> endl);
