...
int main(int argc, char** argv)
{
  ...
  while (!htif->done() && trace_count < max_cycles && !tile.Top_BoomTile_core_dpath__throw_idle_error.lo_word())
  {
    ...
    // look for csrr rd, uarch0 instruction, which only appears as a result
    // of calling setStats().  Yes, this is quite an ugly hack.
    #define RDBITS     0xF80
    #define RDCYCLE    0xC0002073
    #define RD_UARCH_0 0xCC002073
    if (tile.Top_BoomTile_core_dpath__com_valids_0.lo_word() &&
        ((tile.Top_BoomTile_core_dpath__com_uops_0_inst.lo_word() | RDBITS) == (RD_UARCH_0 | RDBITS))
         )
    {
    ...
    *****/
    if (tile.Top_BoomTile_core_dpath__read_uarch0.lo_word()) {
      if (in_test_segment) {
        tracer.stop();
        in_test_segment = false;
      }
  ...
  else if (tile.Top_BoomTile_core_dpath__throw_idle_error.lo_word())
  {
    fprintf(stderr, "*** FAILED *** (pipeline idle error) after %lld cycles\n", (long long)trace_count);
    ret = 4;
  }
