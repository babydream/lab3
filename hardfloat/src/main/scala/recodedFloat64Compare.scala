...
class recodedFloat64Compare_io(SIG_WIDTH: Int, EXP_WIDTH: Int) extends Bundle {
...
class recodedFloat64Compare(SIG_WIDTH: Int = 52, EXP_WIDTH: Int = 12) extends Module {
  override val io = new recodedFloat64Compare_io(SIG_WIDTH, EXP_WIDTH);
        ...
        val a_exp  = io.a(FLOAT_WIDTH-2,SIG_WIDTH).toUInt;
        val a_sig  = io.a(SIG_WIDTH-1,0).toUInt;

        val b_sign = io.b(FLOAT_WIDTH-1);
        val b_exp  = io.b(FLOAT_WIDTH-2,SIG_WIDTH).toUInt;
        val b_sig  = io.b(SIG_WIDTH-1,0).toUInt;
        ...
        // Special case checks
        //
        val neg_zero_eq    = (a_code === UInt("b000",3) && b_code === UInt("b000",3) && ~sign_equal);
        val a_or_b_is_nan  = (a_code === UInt("b111",3) || b_code === UInt("b111",3));
        val signalling_nan = Cat(a_code,io.a(SIG_WIDTH-1)) === UInt("b1110",4) ||
                             Cat(b_code,io.b(SIG_WIDTH-1)) === UInt("b1110",4);
        ...
        io.a_eq_b :=          Mux(neg_zero_eq, Bool(true),
                              ...
                              Mux(a_code === UInt("b111",3), Bool(false),
                              Bool(true))))));
        ...
        io.a_lt_b :=         Mux(a_or_b_is_nan, Bool(false),
                             ...
                             Mux(a_sign === UInt("b0",1) && b_sign === UInt("b0",1),
                               Mux(exp_equal, sig_a_lt_sig_b, exp_a_lt_exp_b),
                             Mux(a_sign === UInt("b1",1) && b_sign === UInt("b0",1), Bool(true),
                             Mux(a_sign === UInt("b0",1) && b_sign === UInt("b1",1), Bool(false),
                             Mux(a_sign === UInt("b1",1) && b_sign === UInt("b1",1),
