...
class estNormDistP53PosSum108_io() extends Bundle{
  val a = UInt(INPUT, 108); 
  val b = UInt(INPUT, 108);     
  val out = UInt(OUTPUT, 8);                      
}

class estNormDistP53PosSum108 extends Module {
    override val io = new estNormDistP53PosSum108_io();                 
    val key = (( io.a ^ io.b ) ^ Cat( io.a | io.b, UInt(0,1)))(107, 0);
/*
    val lower_half =
    ...
    io.out :=
        ...
        lower_half)))))))))))))))))))))))))))))))))))))))))))))))))))));
*/
  io.out := UInt(160, 8) - Log2(key.toUInt, 108);
}
