...
class recodedFloat64ToAny_io(SIG_WIDTH: Int, EXP_WIDTH: Int, INT_WIDTH: Int) extends Bundle {
...
class recodedFloat64ToAny(SIG_WIDTH: Int = 52, EXP_WIDTH: Int = 12, INT_WIDTH: Int = 64) extends Module {
  override val io = new recodedFloat64ToAny_io(SIG_WIDTH, EXP_WIDTH, INT_WIDTH);

    val FLOAT_WIDTH = SIG_WIDTH + EXP_WIDTH + 1;
    val SHIFT_WIDTH = INT_WIDTH + 1; // Save one fraction bit.
    val STAGES = 7;                  // $clog2(SHIFT_WIDTH);
    val EXP_OFFSET = UInt("h800", 12).toUInt; //12'h800; // Recoded offset=2048 (IEEE offset=1023)
...
val isValidShift = (exponent(11,6) === UInt("b100000",6) || isZeroOrOne);
val shift_count =
    Mux(exponent(11,6) === UInt("b100000",6),  Cat(UInt(0,1),~exponent(5,0)),
...
val isValidUnsigned =
    Mux(isValidShift && !sign, (absolute_round(INT_WIDTH-1,0) <= maxInteger), // BCR: LE Op OK?
    Mux(!sign,                 isTiny,
    Bool(false)));

val isValidSigned =
    Mux(isValidShift && !sign, (absolute_round(INT_WIDTH-1,0) <= maxInteger), // BCR: LE Op OK?
    Mux(isValidShift && sign,  (absolute_round(INT_WIDTH-1,0) <= maxInteger+UInt(1)), // BCR: LE Op OK?
    isTiny));
