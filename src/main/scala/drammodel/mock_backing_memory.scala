...
class MockMemory()(implicit conf: MemoryIFConfiguration) extends Module {
  val io = new MemIO().flip
  val memoryLines = scala.math.pow(2, conf.addrBits)
  val backingMemory = Mem(Bits(width = conf.dataBits*conf.dataBeats), 2048)
  ...
  when(currentState === idle){
    ...
    io.resp.bits.data := readData(conf.dataBits - 1, 0)
    ...
    io.resp.bits.tag := io.req_cmd.bits.tag
    io.resp.bits.data := readData(conf.dataBits*2 - 1, conf.dataBits)
    io.resp.valid := Bool(true)
    when(io.resp.ready){
      currentState := read2
    }
    ...
    io.resp.bits.tag := io.req_cmd.bits.tag
    io.resp.bits.data := readData(conf.dataBits*3 - 1, conf.dataBits*2)
    io.resp.valid := Bool(true)
    when(io.resp.ready){
      currentState := read3
    }
    ...
    io.resp.bits.tag := io.req_cmd.bits.tag
    io.resp.bits.data := readData(conf.dataBits*4 - 1, conf.dataBits*3)
    io.resp.valid := Bool(true)
    when(io.resp.ready){
