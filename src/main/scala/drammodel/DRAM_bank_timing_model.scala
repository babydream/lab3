...
class BankCmdIO() extends Bundle {
  val valid = Bool(INPUT)
  val activate = Bool(INPUT)
  val read = Bool(INPUT)
  val write = Bool(INPUT)
  val precharge = Bool(INPUT)
  val bankAddr = UInt(INPUT, width = DRAM_BANK_ADDR_WIDTH)
  val rowAddr = UInt(INPUT, width = DRAM_ROW_ADDR_WIDTH)
  val colAddr = UInt(INPUT, width = DRAM_COL_ADDR_WIDTH) 
}

class BankToCtrlFSMIO() extends Bundle {
  val rowAddr = UInt(OUTPUT, width = DRAM_ROW_ADDR_WIDTH)
}

class BankErrorIO() extends Bundle {
  val invalidCmd = Bool(OUTPUT)
  val tRAS_violation = Bool(OUTPUT)
  val tRCD_violation = Bool(OUTPUT)
  val tRP_violation = Bool(OUTPUT)
  val tCCD_violation = Bool(OUTPUT)
  val tRTP_violation = Bool(OUTPUT)
  val tWTR_violation = Bool(OUTPUT)
  val tWR_violation = Bool(OUTPUT)
}

class BankParameterIO extends Bundle {
  val tRAS = UInt(INPUT, width = TIMING_COUNTER_WIDTH)
  val tRCD = UInt(INPUT, width = TIMING_COUNTER_WIDTH)
  val tRP = UInt(INPUT, width = TIMING_COUNTER_WIDTH)
  val tCCD = UInt(INPUT, width = TIMING_COUNTER_WIDTH) 
  val tRTP = UInt(INPUT, width = TIMING_COUNTER_WIDTH)
  val tWTR = UInt(INPUT, width = TIMING_COUNTER_WIDTH) 
  val tWR = UInt(INPUT, width = TIMING_COUNTER_WIDTH)
}

class DRAMBank extends Module {
  val io = new Bundle {
