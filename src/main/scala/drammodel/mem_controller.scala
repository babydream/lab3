...
class MemController()(implicit conf: MemoryControllerConfiguration) extends Module {
  ...
  Predef.assert(DRAM_BANK_ADDR_WIDTH + DRAM_ROW_ADDR_WIDTH + DRAM_COL_ADDR_WIDTH + conf.addrOffsetWidth <= mif.addrBits)
  ...
  val currentAddr = Reg(init = UInt(0, conf.mif.addrBits))
  val nextAddr = UInt(); nextAddr := currentAddr
  when(io.fireTgtCycle){
    currentAddr := nextAddr
  }
  val readTagQueue = Module(new Queue(Bits(width = conf.mif.tagBits), 16))
  ...
  val dataInBusData = Vec.fill(conf.deviceWidth){new MemData}
  ...
  val writeDatas = Vec.fill(conf.deviceWidth - 1){Reg(new MemData)}
  val nextWriteDatas = Vec.fill(conf.deviceWidth - 1){new MemData}
  ...
  when(currentState === idle){
    when(io.mem_cmd_queue.valid && readDataQueues(0).io.enq.ready && readDataQueues(1).io.enq.ready && readDataQueues(2).io.enq.ready && readDataQueues(3).io.enq.ready){
      when(timer.io.activate_rdy){
        ...
        io.DRAMModel.cmdBus.cmd := activate_cmd
        io.DRAMModel.cmdBus.rowAddr := io.mem_cmd_queue.bits.addr(DRAM_ROW_ADDR_WIDTH + DRAM_COL_ADDR_WIDTH + conf.addrOffsetWidth - 1, DRAM_COL_ADDR_WIDTH + conf.addrOffsetWidth)
        io.DRAMModel.cmdBus.bankAddr := io.mem_cmd_queue.bits.addr(DRAM_BANK_ADDR_WIDTH + DRAM_ROW_ADDR_WIDTH + DRAM_COL_ADDR_WIDTH + conf.addrOffsetWidth - 1, DRAM_ROW_ADDR_WIDTH + DRAM_COL_ADDR_WIDTH + conf.addrOffsetWidth)
    ...
    when(timer.io.read_rdy){
      ...
      io.DRAMModel.cmdBus.bankAddr := currentAddr(DRAM_BANK_ADDR_WIDTH + DRAM_ROW_ADDR_WIDTH + DRAM_COL_ADDR_WIDTH + conf.addrOffsetWidth - 1, DRAM_ROW_ADDR_WIDTH + DRAM_COL_ADDR_WIDTH + conf.addrOffsetWidth)
      io.DRAMModel.cmdBus.colAddr := currentAddr(DRAM_COL_ADDR_WIDTH + conf.addrOffsetWidth - 1, conf.addrOffsetWidth)
      nextState := colOpened
    }
    ...
    when(timer.io.precharge_rdy){
      ...
      io.DRAMModel.cmdBus.rowAddr := currentAddr(DRAM_ROW_ADDR_WIDTH + DRAM_COL_ADDR_WIDTH + conf.addrOffsetWidth - 1, DRAM_COL_ADDR_WIDTH + conf.addrOffsetWidth)
      io.DRAMModel.cmdBus.bankAddr := currentAddr(DRAM_BANK_ADDR_WIDTH + DRAM_ROW_ADDR_WIDTH + DRAM_COL_ADDR_WIDTH + conf.addrOffsetWidth - 1, DRAM_ROW_ADDR_WIDTH + DRAM_COL_ADDR_WIDTH + conf.addrOffsetWidth)
      nextState := idle
    }
